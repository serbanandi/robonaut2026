...
uint8_t uart_Init(uart_UartType* uart, UART_HandleTypeDef* huart, IRQn_Type uartIr, uint8_t readTerminationChar)
{
    ...
    uint8_t ok =
        int_SubscribeToInt(INT_UART_TX_CPLT, (int_CallbackFn) uart_HandleTransmitCplt, (void*) uart, (void*) huart);
    ok = ok &&
         int_SubscribeToInt(INT_UART_RX_CPLT, (int_CallbackFn) uart_HandleReceiveCplt, (void*) uart, (void*) huart);
    __HAL_UART_CLEAR_FLAG(huart, UART_CLEAR_OREF);
    ok =
        ok && HAL_UART_Receive_DMA(uart->huart, (uint8_t*) uart->readCircularBuffer, UART_READ_BUFFER_LENGTH) == HAL_OK;
        ...
        if (uart->startOfWriteData == -1)
        {
            ...
            else
            {
                ...
                else
                {
                    uart->transmissionInProgress = 1;
                    uart->startOfWriteData = 0;
                    ok = ok && HAL_UART_Transmit_DMA(uart->huart,
