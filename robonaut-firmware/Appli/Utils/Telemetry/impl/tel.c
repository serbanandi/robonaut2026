...
static uint8_t tel_AddByteEscaped(uint8_t byte) 
{
    ...
    else 
    {
        ...
        switch(byte)
        {
            case TEL_PROTOCOL_SOF:
                txTmpBuffer[txTmpIndex++] = 0x01;
                break;
            case TEL_PROTOCOL_ESC:
                txTmpBuffer[txTmpIndex++] = 0x02;
                break;
            case TEL_PROTOCOL_EOF:
...
static void tel_ProcessPacket(void) 
{
    ...
    {
        if (currentIndex >= rxByteCnt) return; // Malformed
    ...
    else if ((rxFrameType & TEL_CAT_MASK) == TEL_CAT_STREAM) 
    {
        if ((rxFrameType & TEL_FID_MASK) == TEL_FID_TEXT_INPUT) 
        {
            ...
            uint32_t len = (rxByteCnt - currentIndex) < sizeof(inputTextBuffer) - 1 ? (rxByteCnt - currentIndex) : sizeof(inputTextBuffer) - 1;
...
static void tel_ParseRxByte(uint8_t byte) 
{
    ...
    switch (rxState) 
    {
        ...
        case TEL_RX_ESCAPED:
            ...
            {
                case 0x01:
                    byte = TEL_PROTOCOL_SOF;
                    break;
                case 0x02:
                    byte = TEL_PROTOCOL_ESC;
                    break;
                case 0x03:
                    byte = TEL_PROTOCOL_EOF;
                    break;
                default:
